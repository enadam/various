...
/* Use TABLE(var, ELEM(k1, v1, ELEM(k2, v2, ... ELEM(kn, vn, Zero))))
 * to define static int->int tables. */
#ifndef __cplusplus
  /* Use gcc's designated initializers extension */
# define Zero                   /* Nothing */
# define ELEM(k, v, n)          [k] = v, n
# define TABLE(table, elements) int table[] = { elements }
#else /* __cplusplus, can't use the extension because g++ doesn't have it */
/* Create a constant function which returns the associated value
 * if the requested index is its key, or asks the next element. */
template<int key, int val, typename next>
struct Table
{
  static int valueof(int idx)
    { return idx == key ? val : next::valueof(idx); }
  int operator[](int idx)
    { return valueof(idx); }
};
struct Zero { static int valueof(int idx) { return 0; } };

# define ELEM(k, v, n)          Table<k, v, n>
# define TABLE(table, elements) elements table
#endif /* __cplusplus */
...
#ifdef HAVE_GDK
# define region_t               GdkRegion *
# define create_region(regp)    *(reg) = gdk_region_new()
# define delete_region(regp)    gdk_region_destroy(*reg)
# define clear_region(regp)     gdk_region_xor(*(regp), *(regp))

static void rect_from_xrect(GdkRectangle *rect, XRectangle const *xrect)
{
  rect->x      = xrect->x;
  rect->y      = xrect->y;
...
static void add_rect_to_region(GdkRegion **regp, XRectangle const *xrect)
{
  GdkRectangle rect;
  rect_from_xrect(&rect, xrect);
  gdk_region_union_with_rect(*regp, &rect);
} /* add_rect_to_region */

static Bool is_rect_contained(XRectangle const *xrect,
                              GdkRegion const **regionp)
{
  GdkRectangle rect;
  rect_from_xrect(&rect, xrect);
  return gdk_region_rect_in(*regionp, &rect) == GDK_OVERLAP_RECTANGLE_IN;
} /* is_rect_contained */

static Bool is_region_contained(GdkRegion const **smallerp,
                                GdkRegion const **largerp)
{
  GdkRegion *tmp;
  gboolean is_contained;

  tmp = gdk_region_copy(*smallerp);
  gdk_region_subtract(tmp, *largerp);
  is_contained = gdk_region_empty(tmp);
...
# define QRECT(xrect)                           \
         QRect((xrect)->x, (xrect)->y,          \
               (xrect)->width, (xrect)->height)
# define region_t                               QRegion *
# define create_region(regp)                    *(regp) = new QRegion()
# define delete_region(regp)                    delete *(regp)
# define clear_region(regp)                     **(regp) = QRegion()
# define add_rect_to_region(regp, xrect)        \
         **(regp) += QRECT(xrect)
# define is_rect_contained(xrect, regp)         \
         **(regp).contains(QRECT(xrect))
# define is_region_contained(smallerp, largerp) \
         **(smallerp).subtracted(**largerp).isEmpty()
#else /* ! HAVE_GDK && ! HAVE_QT */
# define HAVE_SIMPLE_REGION_T             /* We're simple. */
# define region_t                         XRectangle
# define create_region(regp)              /* NOP */
# define delete_region(regp)              /* NOP */
# define clear_region(regp)               memset(regp, 0, sizeof(*(reg)))
# define add_rect_to_region(regp, xrect)  *(regp) = *(xrect)
# define is_region_contained(smallerp, largerp)                 \
         /* Is $smaller * $larger == $smaller ? */              \
         intersect(NULL, (largerp)->x - (smallerp)->x,          \
                         (largerp)->width, (smallerp)->width)   \
                   == (smallerp)->width                         \
      && intersect(NULL, (largerp)->y - (smallerp)->y,          \
                         (largerp)->height, (smallerp)->height) \
                   == (smallerp)->height
#endif /* ! HAVE_GDK && ! HAVE_QT */

#ifdef NONONO
struct uevent_st
{
  struct uevent_type_st
    {
      char const *name;
      enum
        {
          CREATED, MAPPED, UNMAPPED, DESTROYED,
          RESHAPED, CONFIGURED, STACKED,
          VISIBILITY, OBSCURED, PARTIALLY_OBSCURED, UNOBSCURED,
          EXPOSED, ALL_EXPOSED, DAMAGED, ALL_DAMAGED,
          PROPERTY, NEWPROP, PROPDEL,
          KEY, KEYDOWN, KEYUP,
        }
      utype;
      enum { NO_ARG, OPT_ARG, HAS_ARG } arg;
      int xtype;
      unsigned event_mask;
    }
  const *type;

  Bool is_child, has_arg;
  ...
  union
  {
    ...
    struct { unsigned state, keycode; };
  };

  Bool matched;
};

struct event_matcher_st
{
  XID damage;
  region_t exposed_region, damaged_region;
  unsigned nevents, matched_events;
  struct uevent_st events[0];
};

static void refresh(struct uevent_st *uevent, unsigned nevents,
                    XRectangle const *shape, unsigned nshape)
{
  for (; nevents > 0; nevents--, uevent++)
    {
      unsigned i;

      if (uevent->type->utype != ALL_EXPOSED
          && uevents->type->utype != ALL_DAMAGED)
        continue;

      clear_region(&uevent->shape);
      for (i = 0; i < nshape; i++)
        {
          if (uevent->has_arg)
            { /* We have a roi to take into account. */
              XRectangle tmp;

              tmp.width = intersect(&tmp.x,
                                    shape[i].x-uevent->roi.x,
                                    shape[i].width, uevent->roi.width);
              tmp.x += uevent->roi.x;
              tmp.height = intersect(&tmp.y,
                                     shape[i].y-uevent->roi.y,
                                     shape[i].height, uevent->roi.height);
              tmp.y += uevent->roi.y;
              add_rect_to_region(&uevent->shape, &tmp);
            }
          else
            /* uevent->shape <- full shape */
            add_rect_to_region(&uevent->shape, &shape[i]);
        }
    }
}

static void refresh1(struct uevent_st *uevents, unsigned nevents,
                     Window win, int x, int y, unsigned w, unsigned h)
{
#ifdef HAVE_SIMPLE_REGION_T
  XRectangle shape;
  ...
  shape.width   = w;
  shape.height  = h;
  refresh(uevents, nevents, &shape, 1);
#else
  int n, m;
  XRectangle *shape;

  shape = XShapeGetRectangles(Dpy, win, ShapeBounding, &n, &m);
  assert(shape != NULL);
  refresh(uevents, nevents, shape, n);
  XFree(shape);
#endif
}

static struct event_matcher_st *prepare_event_matcher(char const *events,
                                                      Bool allocate_state)
{
  static struct uevent_type_st uevent_types[] =
  {
    { "created",            CREATED,              NO_ARG,
       CreateNotify,        SubstructureNotifyMask              },
    { "mapped",             MAPPED,               NO_ARG,      
       MapNotify,           StructureNotifyMask                 },
    { "unmapped",           UNMAPPED,             NO_ARG,      
       UnmapNotify,         StructureNotifyMask                 },
    { "destroyed",          DESTROYED,            NO_ARG,      
       DestroyNotify,       StructureNotifyMask                 },
    /* -------------------------------------------------------- */            
    { "reshaped",           RESHAPED              OPT_ARG       },
    { "configured",         CONFIGURED,           NO_ARG,      
       ConfigureNotify,     StructureNotifyMask                 },
    { "stacked",            STACKED,              HAS_ARG,
       ConfigureNotify,     StructureNotifyMask                 },
    /* -------------------------------------------------------- */            
    { "visibility",         VISIBILITY,           NO_ARG,      
       VisibilityNotify,    VisibilityChangeMask                },
    { "obscured",           OBSCURED,             NO_ARG,      
       VisibilityNotify,    VisibilityChangeMask                },
    { "partially_obscured", PARTIALLY_OBSCURED,   NO_ARG,      
       VisibilityNotify,    VisibilityChangeMask                },
    { "unobscured",         UNOBSCURED,           NO_ARG,      
       VisibilityNotify,    VisibilityChangeMask                },
    /* -------------------------------------------------------- */
    { "exposed",            EXPOSED,              OPT_ARG,
       Exposed,             ExposureMask                        },
    { "all_exposed",        ALL_EXPOSED,          OPT_ARG,
       Exposed,             ExposureMask                        },
    { "damaged",            DAMAGED               OPT_ARG  },
    { "all_damaged",        ALL_DAMAGED           OPT_ARG  },
    /* -------------------------------------------------------- */
    { "property",           PROPERTY,             OPT_ARG,
       PropertyNotify,      PropertyChangeMask                  },
    { "newprop",            NEWPROP,              OPT_ARG,
       PropertyNotify,      PropertyChangeMask                  },
    { "propdel",            PROPDEL,              OPT_ARG,
       PropertyNotify,      PropertyChangeMask                  },
    /* -------------------------------------------------------- */
    { "key",                KEYDOWN,              OPT_ARG,
  ...
  char *what;
  struct event_matcher_st *state;
  unsigned i, maxevents, event_mask;
  Bool track_shape, track_damages, allat;

  /* Estimate how many events the user specified. */
  for (i = 0, maxevents = 1; events[i]; i++)
    if (events[i] == ',')
      maxevents++;
  xmalloc(&state, sizeof(*state) + sizeof(state->events[0])*maxevents);

  /* Parse $str and fill in $events. */
  event_mask = 0;
  track_shape = track_damages = allat = False;
  for (state->nevents = 0; (events = dup_optarg(events, &what)) != NULL;
       state->nevents++)
    {
      char *name, *arg;
      struct uevent_st *uevent;
      struct uevent_type_st *type;

      assert(state->nevents < maxevents);
      event = &state->uevents[state->nevents];

      /* Get the uevent name and parameter. */
      if (!(name = isprefix(what, "child_")))
        name = what;
      if ((arg = strchr(name, '=')))
        *arg++ = '\0';

      /* Find the uevent_type_st for $name. */
      type = NULL;
      for (o = 0; o < MEMBS_OF(uevent_types); o++)
        if (!strcmp(name, uevent_types[o].name))
          {
            type = &uevent_types[o];
            break;
          }
       ...
       * CreateNotify:es are always generated on the parent window. */
      uevent->is_child = type->utype == CREATED || name != what;
      if (uevent->is_child)
        {
      ...
      switch (type->utype)
        {
          default:
            if (arg)
              { /* Pick the child window for child_* events. */
                assert(uevent->is_child && type->arg == NO_ARG);
                must_choose_window(arg, &uevent->child);
              }
            break;
          case RESHAPED:
            track_shape = True;
            if (arg)
              if (!strcmp(arg, "bounding"))
                  uevent->shape_kind = ShapeBounding;
              else if (!strcmp(arg, "input"))
                  uevent->shape_kind = ShapeInput;
              else if (!strcmp(arg, "clip"))
                  uevent->shape_kind = ShapeClip;
            break;
          case STACKED:
            assert(arg != NULL);
            must_choose_window(arg, &uevent->sibling);
            break;
          case DAMAGED: case ALL_DAMAGED:
            track_damages = True; /* Fall through */
          case EXPOSED: case ALL_EXPOSED:
            if (type->utype == ALL_EXPOSED || type->utype == ALL_DAMAGED)
              {
                allat = True;
          ...
          case PROPERTY: case NEWPROP: case PROPDEL:
            if (arg)
              uevent->prop = get_atom(arg);
            break;
          case KEYDOWN: case KEYUP:
            if (arg)
              {
                static struct { char const *mod; unsigned mask; }
                const mods[] =
                {
                  { "b1-",    Button1Mask }, { "b2-",    Button2Mask },
                  { "b3-",    Button3Mask }, { "b4-",    Button4Mask },
                  { "b5-",    Button5Mask },
                  { "shift-", ShiftMask   }, { "ctrl-",  ControlMask },
                  { "lock-",  LockMask    },
                ...
                /* Get the required modifiers. */
                uevent->state = i = 0;
                do
                  if ((rest = isprefix(arg, mods[i].mod)) != NULL)
                    {
                      arg = rest;
                      uevent->state |= mods[i].mask;
                      i = 0;
      ...
      free(what);
    } /* until all of $events is parsed */

  /* Select the events to receive from $win. */
  if (track_shape || allat)
    { /* If we can't track the shape, track the geometry. */
#ifdef HAVE_XEXT
      int shape_error;
...
# ifdef HAVE_SIMPLE_REGION_T
      /* Our simple region_t can only cope with a bounding rectangle. */
      assert(XShapeQueryExtents(Dpy, win,
                                &b, &bx, &by, &bw, &bh,
                                &c, &cx, &cy, &cw, &ch));
# endif /* HAVE_SIMPLE_REGION_T */

      refresh1(state->uevents, state->nevents, win, bx, by, bw, bh);
#endif /* HAVE_XEXT */
    }
  ...
  if (track_damages)
    {
      ...
      /* Look up the damage event base if necessary. */
      if (!damage_event->xtype)
        {
          assert(XDamageQueryExtension(Dpy, &damage_event, &damage_error));
          uevent_types[DAMAGED].xtype     = damage_event;
          uevent_types[ALL_DAMAGED].xtype = damage_event;
        }

      /* We'll have only one Damage per window. */
...
static Window match_event(XEvent const *xevent,
                          struct event_matcher_st *state, Window win)
{
  unsigned i;
  struct uevent_st *uevent;

  /* Find the uevent matching this $xevent. */
  for (i = state->nevents, uevent = state->uevents; i > 0; i--, uevent++)
    {
      Bool matched;
      Window subject;

      /* Can this uevent match by event type? */
      if (uevent->type->xtype != xevent.xany.type)
        continue;

      /* See if the uevent details match the xevent. */
      matched = True;
      subject = xevent.xany.window;
      switch (uevent->type->utype)
        {
          /* CREATED-MAPPED-UNMAPPED-DESTROYED */
          case CREATED:
            subject = xevent.xcreatewindow.window;
            break;
          case MAPPED:
            subject = xevent.xmap.window;
            break;
          case UNMAPPED:
            subject = xevent.xunmap.window;
            break;
          case DESTROYED:
            subject = xevent.xdestroywindow.window;
            break;
          ...
          case RESHAPED:
            {
              ...
              if (!uevent->has_arg)
                  break;
              memcpy(&shev, &xevent, sizeof(shev));
              matched = shev.kind == uevent->shape_kind;
              if (shev.kind == ShapeBounding)
                refresh1(state, win,
                         shev.x, shev.y, shev.width, shev.height);
              break;
            }
#endif /* HAVE_XEXT */
          case STACKED:
            /* Is $subject stacked above $sibling? */
            matched = xevent.xconfigure.above == events[i].sibling;
            /* Fall through */
          case CONFIGURED:
            subject = xevent.xconfigure.window;
            break;
          ...
          case VISIBILITY:
            break;
          case OBSCURED:
            matched = xevent.xvisibility.state == VisibilityFullyObscured;
            break;
          case PARTIALLY_OBSCURED:
            matched = xevent.xvisibility.state == VisibilityPartiallyObscured;
            break;
          case UNOBSCURED:
            matched = xevent.xvisibility.state == VisibilityUnobscured;
            break;
          ...
          default:
            {
              ...
              switch (uevent->type->utype)
                {
                  ...
                  case EXPOSED: ALL_EXPOSED:
                    area.x      = xevent.xexpose.x;
                    area.y      = xevent.xexpose.y;
                    area.width  = xevent.xexpose.width;
                    area.height = xevent.xexpose.height;
                    accu = &state->exposed_region;
                    break;
#ifdef HAVE_XDAMAGE
                  case DAMAGED: ALL_DAMAGED:
                    {
              ...
              switch (uevent->type->utype)
                {
                  default: assert(False);
                  case EXPOSED: case DAMAGED:
                    if (uevent->has_arg)
                      /* Is any of uevent.roi exposed/damaged? */
                      matched = intersect(NULL,
                                          area.x-uevent->roi.x,
                                          area.width, uevent->roi.width)
                             && intersect(NULL,
                                          area.y-uevent->roi.y,
                                          area.height, uevent->roi.height);
                    break;
                  case ALL_EXPOSED: case ALL_DAMAGED:
                    /* accu += area, matched := shape <= accu */
                    add_rect_to_region(accu, &area);
                    matched = is_region_contained(&uevent->shape, accu);
                    break;
                }
              break;
            }
          ...
          /* PROPERTY changed */
          case PROPERTY:
            matched  = !uevent->has_arg
                    || xevent.xproperty.atom == uevent->prop;
            break;
          case NEWPROP:
            matched  = xevent.xproperty.state == PropertyNewValue
                    && (!uevent->has_arg
                        || xevent.xproperty.atom == uevent->prop);
            break;
          case PROPDEL:
            matched  = xevent.xproperty.state == PropertyDelete
                    && (!uevent->has_arg
                        || xevent.xproperty.atom == uevent->prop);
            break;
          ...
          case KEYDOWN:
            matched  = !uevent->has_arg
                    || ((xevent.xkey.state & uevent->state) == uevent->state
                        && xevent.xkey.keycode == uevent->keycode);
            break;
        }
      ...
      /* Did $xevent happen on the expected window? */
      if (!uevent->is_child)
        /* We expect the $xevent to be about $win itself. */
        matched = subject == win;
      else if (uevent->type->utype != STACKED && uevent->has_arg)
        /* uevent expects events about a specific child window. */
        matched = xevent.xany.window == win && subject == uevent->child;
      else
        /* $xevent is expected about an unspecified child of $win. */
        matched = xevent.xany.window == win && subject != win;
          ...
          if (!uevent->matched)
            {
              ...
              assert(state->matchied_events <= state->nevents);
            }
          return subject;
        }
    } /* for all $events */
          ...
          case 'e':
            {
              // [any|all] [propagate]
              // {created|mapped|unmapped|destroyed}
              // {child_mapped|child_unmapped|child_destroyed}
              // {configured|child_configured}
              // {visibility|obscured|partially_obscured|unobscured}
              // {exposed|damaged}[=<geo>]
              // {property|newprop|propdel}[=<property>]
              break;
            }
